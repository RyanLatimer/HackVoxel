...
bool VoxelChunk::isAir(int x, int y, int z) const
{
    ...
    return blocks[x][y][z] == 0; // Return true if the block is air
}
...
void VoxelChunk::generateMesh()
{
    ...
    for (int x = 0; x < CHUNK_SIZE; x++)
    {
        for (int y = 0; y < CHUNK_SIZE; y++)
        {
            ...
            {                if (blocks[x][y][z] == 0)
                ...
                if (isAir(x, y, z + 1))
                {
                    addFace(x, y, z + 1, x + 1, y, z + 1, x + 1, y + 1, z + 1, x, y + 1, z + 1, indexOffset);
                }

                // Back face (negative Z)
                if (isAir(x, y, z - 1))
                {
                    addFace(x + 1, y, z, x, y, z, x, y + 1, z, x + 1, y + 1, z, indexOffset);
                }

                // Right face (positive X)
                if (isAir(x + 1, y, z))
                {
                    addFace(x + 1, y, z, x + 1, y, z + 1, x + 1, y + 1, z + 1, x + 1, y + 1, z, indexOffset);
                }

                // Left face (negative X)
                if (isAir(x - 1, y, z))
                {
                    addFace(x, y, z + 1, x, y, z, x, y + 1, z, x, y + 1, z + 1, indexOffset);
                }

                // Top face (positive Y)
                if (isAir(x, y + 1, z))
                {
                    addFace(x, y + 1, z, x + 1, y + 1, z, x + 1, y + 1, z + 1, x, y + 1, z + 1, indexOffset);
                }

                // Bottom face (negative Y)
                if (isAir(x, y - 1, z))
                {
                    addFace(x, y, z + 1, x + 1, y, z + 1, x + 1, y, z, x, y, z, indexOffset);
                }
            }
        }
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(float), vertices.data(), GL_STATIC_DRAW);    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
                         ...
                         unsigned int& indexOffset)
{
    ...
    float face[] = {
        ...
        x2, y2, z2, 1.0f, 0.0f,  // bottom-right
        x3, y3, z3, 1.0f, 1.0f,  // top-right
        x4, y4, z4, 0.0f, 1.0f   // top-left
    };
